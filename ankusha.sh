...
get_node_metrics() {
    ...
    nm_mem_total=${rm:-0}; nm_mem_free=${fm:-0}
...
draw_quick_stats() {
    ...
    # FIX: single squeue call per state — count extracted from one read.
    ...
    local gu; gu=$(squeue $sq -t R -h -o "%b" 2>/dev/null \
...
draw_queue_section() {
    ...
    local sqf=""; [ "$all_flag" = "false" ] && sqf="-p $partition"
...
main() {
    ...
    while true; do
        # FIX: "tput cup 002>/dev/null" was a typo — "00" was passed as the
        # row arg and "2>/dev/null" as the column arg. Fixed to two separate
        # positional args with proper redirection.
        tput cup 0 0 2>/dev/null
        draw_dashboard "$partition" "$target_user" "interactive" "$all_flag"
        tput ed 2>/dev/null
        # FIX: track sleep PID so cleanup() reaps it on Ctrl+C instead of
        # leaving it as an orphan for the remainder of the refresh interval.
        sleep "$refresh" &
        _SLEEP_PID=$!
