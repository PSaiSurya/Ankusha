...
# ==============================================================================
# WAIT TIME CALCULATOR (Full Duration Display)
# ==============================================================================
calc_wait_time() {
    local submit_time="$1"
    [ -z "$submit_time" ] && { printf '-'; return; }
    
    local now_epoch submit_epoch
    ...
    if [ -z "$submit_epoch" ] || [ "$submit_epoch" = "0" ]; then
        submit_epoch=$(date -j -f "%Y-%m-%dT%H:%M:%S" "$submit_time" +%s 2>/dev/null)
    fi
    
    # Final fallback: try parsing with TZ
    if [ -z "$submit_epoch" ] || [ "$submit_epoch" = "0" ]; then
        submit_epoch=$(TZ=UTC date -d "$submit_time" +%s 2>/dev/null)
    fi
    ...
    local diff_sec=$(( now_epoch - submit_epoch ))
    [ "$diff_sec" -lt 0 ] && diff_sec=0
    
    # Breakdown into years, days, hours, minutes
    local weeks=$(( diff_sec / 604800 ))
    diff_sec=$(( diff_sec % 604800 ))
    ...
    # Calculate years (approximate for display)
    local years=$(( (weeks * 7 + days) / 365 ))
    local remaining_days=$(( (weeks * 7 + days) % 365 ))
    
    # Build output string with explicit units
    local result=""
    if [ "$years" -gt 0 ]; then
        result="${result}${years}y "
        [ "$remaining_days" -gt 0 ] && result="${result}${remaining_days}d "
...
# Strip ANSI escape sequences for length measurement only
visible_len() {
    local s="$1"
...
section_header() {
    ...
    if [ -n "$subtitle" ]; then
        ...
        local slen; slen=$(visible_len "$subtitle")
        local pad_count=$(( inner - tlen - slen - 4 ))
...
load_node_cache() {
    ...
    # ── FIX: Single sinfo call with full format ──────────────────────────────
    local sinfo_fmt="-N -h -o %n|%t|%C|%m|%G"
    [ "$all_flag" = "false" ] && sinfo_fmt="-p $partition $sinfo_fmt"
    
    NODE_INFO_CACHE=$(timeout 15s sinfo $sinfo_fmt 2>/dev/null)
    
    # ── Extract node list from cached data ───────────────────────────────────
    local node_list
    node_list=$(printf '%s' "$NODE_INFO_CACHE" | cut -d'|' -f1 | sort -u | tr '\n' ',')
    node_list="${node_list%,}"

    # ── Fetch detailed node metrics ──────────────────────────────────────────
...
show_help() {
    ...
    _hf "--override-mem SIZE"   "Override node memory total (e.g., 1TB, 1024GB)"
    _hf ""                      "Use when Slurm RealMemory is incorrect"
...
draw_header() {
    ...
    [ "$mode" = "interactive" ] \
        ...
        || badge="[ SNAPSHOT ]"
    local info_len=$(( 2 + 2 + 5 + ${#target_user} + 3 + 2 + 11 + \
                       ${#partition} + 3 + 2 + ${#ts} + 1 + \
                       ${#ACTIVE_TZ_LABEL} + 1 ))
...
draw_quick_stats() {
    ...
    local sq="-h"
    [ "$all_flag" = "false" ] && sq="-p $partition $sq"

    # ── FIX: Use cached sinfo data ───────────────────────────────────────────
    local ni=0 na=0 nm=0 nd=0
    local _eval_str
    _eval_str=$(printf '%s' "$NODE_INFO_CACHE" | awk -F'|' '
        $2 ~ /^idle/  { ni++ }
        $2 ~ /^alloc/ { na++ }
        $2 ~ /^mix/   { nm++ }
        $2 ~ /^down/  { nd++ }
    ...
    # ── Single squeue call for running jobs ──────────────────────────────────
    local _rq_data
    _rq_data=$(squeue $sq -t R -h -o "%b" 2>/dev/null)
    local jr
    jr=$(printf '%s\n' "$_rq_data" | grep -c '.' || echo 0)

    # ── Fetch pending jobs once ───────────────────────────────────────────────
    PENDING_JOBS_DATA=$(squeue $sq -t PD -h -o "%i|%u|%V|%C|%m|%b|%D|%r" 2>/dev/null)
    # Changed %M (time used) to %V (submit time)
    local jp
    jp=$(printf '%s\n' "$PENDING_JOBS_DATA" | grep -c '.' || echo 0)

    # ── GPU totals from NODE_CACHE ────────────────────────────────────────────
    local gt
...
$(pad "GPU (Count)"    "$w_gpu_col" c)"
    ...
    while IFS='|' read -r node state _c _m _g; do
        ...
        printf '\n'
    done < <(printf '%s' "$NODE_INFO_CACHE" | sort -V)
...
draw_queue_section() {
    ...
    # ── CRITICAL FIX: Use process substitution with grep filter (not <<<) ──────
    local pq_data="$PENDING_JOBS_DATA"
    
    pt=$(printf '%s' "$pq_data" | tr -d '\r' | sed '/^[[:space:]]*$/d' | grep -c . 2>/dev/null || echo 0)
    pu=$(printf '%s' "$pq_data" | tr -d '\r' | sed '/^[[:space:]]*$/d' \
        | awk -F'|' -v u="$target_user" '$2==u && $1 ~ /^[0-9]+$/' | wc -l | tr -d ' ')
    [[ "$pt" =~ ^[0-9]+$ ]] || pt=0
    [[ "$pu" =~ ^[0-9]+$ ]] || pu=0
    ...
    local wJID=12 wUSER=14 wWT=16 wPRI=10
    local fixed=$(( 2+wJID+wUSER+wWT+wPRI ))
    local rem=$(( w-fixed )); [ "$rem" -lt 40 ] && rem=40
    local wREQ=$(( rem*52/100 )) wREA=$(( rem-wREQ ))
    [ "$wREQ" -lt 22 ] && wREQ=22
    [ "$wREA" -lt 18 ] && wREA=18
...
$(pad "REASON"    "$wREA"  l)"
    ...
    local count=0
    while IFS='|' read -r jid juser submit_time cpus mem gres num_nodes reason; do
        # ── STRICT VALIDATION ──────────────────────────────────────────────────
        [ -z "$jid" ] && continue
        [[ ! "$jid" =~ ^[0-9]+$ ]] && continue
        [ -z "$juser" ] && continue
        ...
        local res pri pc uc wt
        # ── FIX: Strip ONLY newlines/carriage returns, keep spaces in time format ─
        wt=$(calc_wait_time "$submit_time" | tr -d '\r')
        wt="${wt%$'\n'}"
        res=$(build_res_str "$cpus" "$mem" "$gres" "$num_nodes")
        
        pri=$(printf '%s' "$sprio_data" | awk -v id="$jid" '$1==id { print $2; exit }')
        [ -z "$pri" ] && pri="-"
        if [[ "$pri" =~ ^[0-9]+$ ]]; then
        ...
        printf '  %s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s\n' \
            "$C_VALUE" "$(pad "$jid"    "$wJID"  l)" "$C_RESET" \
            "$uc"      "$(pad "$juser"  "$wUSER" l)" "$C_RESET" \
            ""         "$(pad "$wt"     "$wWT"   l)" \
            "$pc"      "$(pad "$pri"    "$wPRI"  l)" "$C_RESET" \
            "$C_GPU"   "$(pad "$res"    "$wREQ"  l)" "$C_RESET" \
            "$C_MUTED" "$(pad "$reason" "$wREA"  l)" "$C_RESET"
    done < <(printf '%s' "$pq_data" | tr -d '\r' | sed '/^[[:space:]]*$/d')
...
draw_footer() {
    ...
    # Build the content string, then measure and pad
    local left_str=""
    if [ "$mode" = "interactive" ]; then
        left_str=" ▶ Refreshing every ${REFRESH:-$DEFAULT_REFRESH}s  Ctrl+C to exit"
    else
        left_str="■ Snapshot"
    fi
    ...
    left_str="${left_str}  ◈ ${part_label}  ⊙ ${ACTIVE_TZ_LABEL}"

    local right_str="${TOOL_NAME} v${VERSION}"

    # Calculate visible lengths for padding
    local llen; llen=$(visible_len "$left_str")
    local rlen; rlen=$(visible_len "$right_str")
    local mid_pad=$(( inner - llen - rlen - 2 ))
    [ "$mid_pad" -lt 1 ] && mid_pad=1

    printf '%s│%s  ' "$C_HEADER" "$C_RESET"
    if [ "$mode" = "interactive" ]; then
        printf '%s ▶ Refreshing every %ds%s  %sCtrl+C to exit%s' \
    ...
    printf '%*s' "$mid_pad" ''
    printf '%s%s%s' "$C_MUTED" "$right_str" "$C_RESET"
    printf '%s│%s\n' "$C_HEADER" "$C_RESET"
