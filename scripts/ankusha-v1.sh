...
draw_queue_section() {
    ...
    # ── CRITICAL FIX: Use process substitution with grep filter ────────────────
    ...
    while IFS='|' read -r jid juser submit_time cpus mem gres num_nodes reason; do
        ...
        wt=$(calc_wait_time "$submit_time")
        wt="${wt%$'\n'}"      # Strip ONLY trailing newline from time
        wt="${wt%$'\r'}"
        res=$(build_res_str "$cpus" "$mem" "$gres" "$num_nodes")
        ...
        [ -z "$pri" ] && pri="-"
        if [[ "$pri" =~ ^[0-9]+$ ]]; then
            [ "$pri" -ge 1000 ] && pc="$C_SUCCESS" || { [ "$pri" -ge 500 ] && pc="$C_WARNING2" || pc="$C_MUTED"; }
        else pc="$C_MUTED"; fi
    ...
    # ── FIX: NO trailing printf '\n' here - dashboard adds it! ──────────────
}
# ==============================================================================
# FOOTER
# ==============================================================================
